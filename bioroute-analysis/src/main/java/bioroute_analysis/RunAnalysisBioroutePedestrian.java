...
public class RunAnalysisBioroutePedestrian {
	
	private static final Logger log = Logger.getLogger(RunAnalysisBioroutePedestrian.class);
	 ...
	 *                             inputChosenFile:       input id|origin|destination|links chosen route tab separated table (String)
	...
	public static void main(String[] args) throws IOException {
		if (args.length != 4) {
			...
			log.error("       inputChosenFile:       input id|origin|destination|links chosen route tab seperated table (String)");
			log.error("       inputBiorouteOutputFile:       input id|origin|destination|links alternatives comma separated table (String)");
		...
		log.info("inputChosenFile:       "+inputChosenFile);
		...
		Map<String, Triple<Node, Node, Path>> ods_chosenRoutes = ParseAlternatives.parseODandChosenRoutes(inputChosenFile,network);
		...
		AnalysisBioroutePedestrian.analysis(outputPathSetFile, network, ods_chosenRoutes, ods_alternatives);
