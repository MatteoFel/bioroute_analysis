...
public class PathXMLWriter implements PathWriter {
	...
	private final Map<List<BasicLink>, Double> ids = new HashMap<>();
	
	private final Map<Double, Double> frequency = new HashMap<>();
	// -------------------- CONSTUCTION --------------------
	...
	public void writePath(final List<BasicLink> linkPath,
		...
		if (!this.pathsSoFar.contains(linkPath)) {
			this.ids.put(linkPath, (double) pathsSoFar.size());
			this.frequency.put(ids.get(linkPath), 1.0);
		}
		
		if (this.skipDuplicates && this.pathsSoFar.contains(linkPath)) {
			this.frequency.put(ids.get(linkPath), this.frequency.get(ids.get(linkPath)) + 1);
			return;
		}
		...
		this.writer.print("id=\"" + ids.get(linkPath) + "\" ");
	...
	public void endOdPair() {
		final StringBuffer ids = new StringBuffer("id=\"");
		final StringBuffer freqs = new StringBuffer("freq=\"");
		for (Entry<Double, Double> id_freq : frequency.entrySet()) {
			Double id = id_freq.getKey();
			Double freq = id_freq.getValue();
			ids.append(id);
			ids.append(" ");
			freqs.append(freq);
			freqs.append(" ");
		}
		this.writer.println("    <frequencies " + ids + "\" " + freqs + "\"/>");
